FROM jenkinsci/jenkins:2.82

LABEL maintainer="Eddú Meléndez" \
      jenkins.version="2.82"

RUN /usr/local/bin/install-plugins.sh blueocean:1.2.4 slack:2.3 pipeline-model-definition:1.2.2 \
...
cloudbees-folder:6.1.2 github-organization-folder:1.6 matrix-auth:1.7 matrix-project:1.12 \
